...
FROM verybigthings/elixir:1.11.2 AS build
...
COPY config/prod.exs config/config.exs .
...
COPY --chown=${APP_USER} --from=pre-release /opt/cache/_build/prod/rel/launch .

USER ${APP_USER}

CMD trap 'exit' INT; ${WORKDIR}/bin/launch start
